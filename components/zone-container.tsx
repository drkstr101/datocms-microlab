...
import styles from "./zone-container.module.css"
...
export default function ZoneContainer({ zone, allZones }: Props) {
  const response = useSWR("/api/stages", {
    initialData: allZones,
    refreshInterval: 5000,
  })

  const updatedZones = response.data || []
  const updatedZone = updatedZones.find((s: Zone) => s.slug === zone.slug) || zone
  ...
  return (
    <div className={styles.container}>
        ...
        {loginStatus === "loggedIn" ? (
          <div className={cn(styles.stream, styleUtils.appear, styleUtils["appear-first"])}>
            <iframe
              ...
              allowFullScreen
              frameBorder="0"
              src={`${updatedZone.stream}?autoplay=1&mute=1`}
      ...
      <ScheduleSidebar allZones={updatedZones} />
    </div>
  )
}
