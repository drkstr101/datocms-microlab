...
function CompanyPosts({ posts }: Props) {
  return (
        ...
        >
          <div className={styles.cardBody}>
            <div>
              ...
              <p className={styles.company}>{post.companyName}</p>
...
export default function PostsGrid({ posts }: Props) {
  const companies = posts.reduce((allCompanies: any, post) => {
    allCompanies[post.companyName] = [...(allCompanies[post.companyName] || []), post]
    return allCompanies
  }, {})
    ...
    <>
      {Object.keys(companies).map((companyName: string) => (
        <div key={companyName} className={styles.companyRow}>
          ...
          <CompanyPosts posts={companies[companyName]} />
        </div>
      ))}
    </>
  )
